...
namespace SetupExportParser.SetupModels.LMSC
{
    ...
    public class LMSCSetup : Logic.Setup, ILMSCSetup
    {

        public string TapeConfiguration { get; set; }

        private ILMSCRearSuspension _lrSuspension;
        ILMSCRearSuspension ILMSCSetup.LRSuspension
        {
            get { return _lrSuspension; }
            set { _lrSuspension = value; }
        }
        private ILMSCRearSuspension _rrSuspension;
        ILMSCRearSuspension ILMSCSetup.RRSuspension
        {
            get { return _rrSuspension; }
            set { _rrSuspension = value; }
        }

        public LMSCSetup(string data) : base(data)
        {
        }
        ...
        protected override void Initialize(string[] dataLines)
        {
            var frontData = dataLines[24].Split('|');
            TapeConfiguration = frontData[11].Split(':')[1].Trim();
        }
    }
}
namespace iRacing.SetupLibrary
{
    ...
    public partial class LMSCSetup : Setup
    {

        public string TapeConfiguration { get; set; }
        
        public LMSCSetup(string htmlExport) : base(htmlExport)
        {
        }
